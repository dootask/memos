 ...
 * embedding DooTask shell for a fresh user token and re-bootstraps at
 * `/?token=…&reauth=1`. DooTask delivers data to a plain iframe (no
 * `window.microApp`) over postMessage: it auto-posts `MICRO_APP_INJECT`
 * (`message.props.userToken`, read live from the host store) on every iframe
 * load, and re-posts it whenever the iframe sends `MICRO_APP_READY`. So we listen
 * for the inject and also solicit one. The `reauth=1` marker is a one-shot guard:
 * if it's already present (the re-bootstrap also failed), or we're not embedded,
 * or no token arrives in time, we fall back to the static "reopen from DooTask"
 * message — so a genuinely dead session can never spin in a reload loop.
 */
...
(function(){
  ...
  var done=false;
  function go(tok){if(done||!tok||typeof tok!=='string')return;done=true;location.replace(location.pathname+'?token='+encodeURIComponent(tok)+'&reauth=1');}
  // DooTask posts MICRO_APP_INJECT (with a live userToken) on load and on demand.
  window.addEventListener('message',function(e){
    var d=e&&e.data;
    if(d&&d.type==='MICRO_APP_INJECT'&&d.message&&d.message.props)go(d.message.props.userToken);
  });
  // Solicit a fresh inject in case the auto one fired before this listener attached.
  try{window.parent.postMessage({type:'MICRO_APP_READY',message:{supportBeforeClose:false}},'*');}catch(e){}
